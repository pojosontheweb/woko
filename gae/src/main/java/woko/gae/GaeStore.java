...
public class GaeStore implements ObjectStore {
    ...
    public Object load(String className, final String key) {
        ...
        return getEm().find(clazz, convertKey(clazz, key));
    }

    private Object convertStringKeyToItsActualType(Class<?> actualType, String keyString) {
        if (actualType.equals(String.class)) {
            return keyString;
        }
        // TODO make use of Stripes type converters ?
        if (int.class.isAssignableFrom(actualType) || Integer.class.isAssignableFrom(actualType)) {
            return Integer.parseInt(keyString);
        } else if (long.class.isAssignableFrom(actualType) || Long.class.isAssignableFrom(actualType)) {
            return Long.parseLong(keyString);
        } else {
            throw new UnsupportedOperationException("unable to convert string key " + keyString + " to its actual type " + actualType);
        }
    }

    private Object convertKey(Class<?> clazz, String key) {
        ...
        if (idField != null) {
            return convertStringKeyToItsActualType(idField.getType(), key);
        } else {
            ...
            return convertStringKeyToItsActualType(idPropReadMethod.getReturnType(), key);
        }
    }
