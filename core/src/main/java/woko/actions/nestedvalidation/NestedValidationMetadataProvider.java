                ...
                for (PropertyDescriptor pd : pds) {
                    ...
                    if (nested != null) {
                        ...
                        if (validates != null && validates.length == 0) {
                            ...
                            // check for @DontValidate on action bean event handler
                            ActionBean ab = actionBeanHolder.get();
                            boolean dontValidate = false;
                            if (ab instanceof WokoActionBean) {
                                WokoActionBean wokoActionBean = (WokoActionBean)ab;
                                Method handler = wokoActionBean.getEventHandlerMethod();
                                if (handler.getAnnotation(DontValidate.class)!=null) {
                                    dontValidate = true;
                                }
                            }
